...
void	print_action(t_philo_act action, t_philo *philo, bool debug)
{
	...
	if (debug)
		return ;	
		//print_action_debug(action, philo, elapsed_t);
	else
	{
...
void	usleep_redux(long sleep_t, t_dinner *dinner)
{
	...
	start_t = get_current_time(MICROSECOND);
	while (sleep_t > get_current_time(MICROSECOND) - start_t)
	{
		if (sim_finished(dinner))
			break ;
		elapsed_t = get_current_time(MICROSECOND) - start_t;
